...
document.addEventListener("DOMContentLoaded", ()=> {
    ...
    turma_menu.addEventListener("click",()=>{
        if(turma_menu.classList.contains("opacity40")){
            toggleopacity(turma_menu)
        }
        ...
        if(!eventos_menu.classList.contains("opacity40")){
            toggleopacity(eventos_menu)
        }
    ...
    // console.log(tier2.value)

    if(tier2.value != 0){
        let t2options = tier2.options

        for(let i=0; i<t2options.length; i++){
            if(t2options[i].getAttribute('tier1') != tier1.value){
                t2options[i].style.display = 'none';
            }
        }
    ...
    delegado.addEventListener("change",()=>{
        let subdelegado_list = document.querySelectorAll('.turma_subdelegado option')

        subdelegado_list.forEach(item_subdelegado =>{
            item_subdelegado.disabled = false;

        })
        subdelegado_list.forEach(item_subdelegado =>{
            if(item_subdelegado.value == delegado.value){
                item_subdelegado.disabled = true;
            }
        })
    })
    subdelegado.addEventListener("change",()=>{
        let delegado_list = document.querySelectorAll('.turma_delegado option')

        delegado_list.forEach(item_delegado => {
            item_delegado.disabled = false;
        })
        delegado_list.forEach(item_delegado => {
            if(item_delegado.value == subdelegado.value){
                item_delegado.disabled = true;
            }
        })
    })
    ...
    const btn_atualizar = document.querySelector('.turma_edit_btns button:nth-of-type(1)')
    btn_atualizar.addEventListener("click", ()=>{

        let student_items = document.querySelectorAll('.coluna_selecionar_alunos_select option')
        student_items.forEach(elem=>{
            elem.selected = 'selected';
        })

        let professor_items = document.querySelectorAll('.coluna_selecionar_professores_select option')
        professor_items.forEach(elem=>{
            elem.selected = 'selected';
        })
